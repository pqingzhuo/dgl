...
class Conv1d(nn.Module):
    def __init__(self, inplane, outplane, Linear=BiLinear):
        super().__init__()
...
class EmaMaxPool(nn.Module):
    def __init__(self, kernel_size, affine=True, Linear=BiLinearLSR, use_bn=True):
        super(EmaMaxPool, self).__init__()
        # self.conv3 = Conv1d(1024, 1024, Linear)
        self.pool = 'ema-max'
...
class PointNetCls(nn.Module):
    def __init__(self, output_classes, input_dims=3, conv1_dim=64,
                 dropout_prob=0.5, use_transform=True, Linear=BiLinearLSR):
        ...
        if use_transform:
            self.transform1 = TransformNet(input_dims, Linear=Linear)
            self.trans_bn1 = nn.BatchNorm1d(input_dims)
            self.transform2 = TransformNet(conv1_dim, Linear=Linear)
...
class TransformNet(nn.Module):
    def __init__(self, input_dims=3, conv1_dim=64, Linear=BiLinearLSR):
